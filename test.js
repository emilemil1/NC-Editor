...
$(function(){
    ...
    DBox.get("btn_box1").setMargin("25px", "top").setShadow(0.4);
    let box = DBox.get("dyn1");
    box.setContent($("<div><div>").text("Frame 2"), 1);
    box.setContent($("<div><div>").text("Frame 3"), 2);
    box.setContent($("<div><div>").text("Frame 4"), 3);
...
function buttonClicked(val){
    ...
    if(box.currentFrame != val && fading != true) {
        fading = true;
        box.fade(0,600).delay(400).setFrame(val).delay(400).fade(1,600).runFunction(function(){fading = false});
    }
}
